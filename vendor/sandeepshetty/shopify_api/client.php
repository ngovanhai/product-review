...
/* 		$seconds_in_a_day = 24 * 60 * 60;
	...
	function omega_client($shop, $shops_token, $api_key, $shared_secret, $private_app=false)
    {
        ...
        return function ($method, $path, $params=array(), &$response_headers=array()) use ($baseurl, $shops_token)
        {
            ...
            $key = md5($baseurl.$method.$path.base64_encode(json_encode($params)));
            $store_path = CACHE_PATH.$shop;
            if (!file_exists($store_path)) {
                mkdir($store_path, 0777, true);
            }
            $key_path = $store_path.'/'.$key;
            if(is_file($key_path)  && !in_array($method, array('POST','PUT'))){ // 30*60
                $data_cache = file_get_contents($key_path);
				return json_decode($data_cache,true);
            }
            $result = _api($method, $url, $query, $payload, $request_headers, $response_headers); 
            file_put_contents($key_path,is_array($result)?json_encode($result):$result);
            return $result; 
        };
    }
	...
	{ 
         // save cache data shop (product, collection...)
    //    return omega_client($shop, $shops_token, $api_key, $shared_secret, $private_app=false); 
		$password = $shops_token;
		$baseurl = "https://$shop/"; 
		return function ($method, $path, $params=array(), &$response_headers=array()) use ($baseurl, $shops_token)
		{
